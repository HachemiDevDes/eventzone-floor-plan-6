...
import { 
  ...
  Plus, Pipette, Box, SeparatorHorizontal, ArrowLeft, ArrowUpRight, Shield, Fence, MapPin, Presentation, Tablet, Video, Wifi, Compass, GlassWater, LayoutTemplate,
...
export default function FloorPlanModifier({ 
  ...
  const renderDirectoryList = (isMobile = false) => {
    const allMatched = elements.filter(el => {
      ...
      if (!previewSearchQuery) return el.type.startsWith("booth") || el.type === "stage" || el.type === "table-chairs" || el.type === "stage-podium" || el.type.startsWith("net") || el.type.startsWith("utility") || el.type.startsWith("access") || el.type === "entrance" || el.type === "exit" || el.type === "desk";
      ...
      if (el.exhibitorId && exhibitors) {
        const ex = exhibitors.find(e => e.id === el.exhibitorId || String(e.id) === String(el.exhibitorId));
        if (ex && ex.name.toLowerCase().includes(q)) return true;
      }
      if ((el.type === "table-chairs" || el.type === "stage-podium") && el.assignments && attendees) {
        const hasMatchingAttendee = Object.values(el.assignments).some(attId => {
          const att = attendees.find(a => String(a.id) === String(attId));
                        ...
                        <div className="bg-slate-50 border-x border-b border-indigo-600/35 p-3.5 rounded-b-xl flex flex-col gap-2 shadow-inner">
                          ...
                          <div className="flex flex-col gap-1.5">
                            {Array.from({ length: el.chairsCount || 6 }).map((_, idx) => {
                              ...
                              const attendee = assignedId ? attendees.find(a => String(a.id) === String(assignedId)) : null;
                              return (
                                <div 
                                  key={idx} 
...
return (
    <div className={`flex flex-col flex-1 bg-slate-50 select-none ${
      ...
      <header className={`sticky top-0 z-50 border-b border-slate-200 bg-white flex ${
        initialPreviewMode 
          ? "flex-col justify-center px-6 pt-6 pb-4 h-auto gap-2 items-start" 
            ...
            <div className="flex-1 relative overflow-hidden flex flex-col w-full h-full">
              ...
              {isPreviewMode && previewDeviceMode === "mobile" && (
                <button
                  ...
                  className="absolute top-4 left-4 bg-indigo-650 hover:bg-indigo-700 text-white font-bold text-[11px] py-2.5 px-4 rounded-full shadow-lg flex items-center gap-1.5 z-40 cursor-pointer transition-transform hover:scale-105 active:scale-95"
                >
                  <Users size={12} />
                  <span>Directory</span>
                </button>
              ...
              <AnimatePresence>
                {isPreviewMode && previewDeviceMode === "mobile" && selectedIds.length === 1 && (() => {
                  const el = elements.find(item => item.id === selectedIds[0]);
                  if (!el) return null;

                  let exhibitor = null;
                  if (el.exhibitorId && exhibitors) {
                    exhibitor = exhibitors.find(ex => ex.id === el.exhibitorId || String(ex.id) === String(el.exhibitorId));
                  }
                  ...
                  if (effectiveSeatId) {
                    ...
                    if (occupantId) {
                      attendee = attendees.find(a => String(a.id) === String(occupantId));
                    }
                        ...
                        <div className="flex gap-3 items-center">
                          {effectiveSeatId ? (
                            attendee && attendee.image ? (
                              <img src={attendee.image} className="w-12 h-12 rounded-xl object-cover shadow border border-slate-100 bg-white shrink-0" alt={attendee.name} />
                            ) : (
                              <div className={`w-12 h-12 rounded-xl bg-gradient-to-tr ${logoGradient} flex items-center justify-center text-white shadow shrink-0`}>
                                <Armchair size={22} className="text-white" />
                              </div>
                            )
                          ) : exhibitor && exhibitor.logo ? (
                            <img src={exhibitor.logo} className="w-12 h-12 rounded-xl object-cover shadow border border-slate-100 bg-white" alt={exhibitor.name} />
                          ) : (() => {
                          ...
                          <div className="flex flex-col gap-0.5 max-w-[190px]">
                            ...
                            <span className="text-[10px] font-semibold text-indigo-650 truncate">{subtitle}</span>
                          </div>
                        </div>
                        <button 
                        ...
                        <div className="flex flex-col gap-1">
                          ...
                          {(() => {
                            ...
                            if (selectedElement?.type === "furniture-chair") {
                              ...
                              if (assignedId) {
                                occupant = attendees.find(a => String(a.id) === String(assignedId));
                              }
